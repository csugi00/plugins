...
namespace Turbo.Plugins.Zy
{
    public class OculusColor : BasePlugin, IInGameWorldPainter, ICustomizer
    {
        ...
        public void PaintWorld(WorldLayer layer)
        {
            ...
            IWorldCoordinate PlayerPosition = Hud.Game.Players.First().FloorCoordinate;
            int WizardsIngame = 0;
            ...
            foreach (var player in Hud.Game.Players)//others
            {
                ...
                {
                    PlayerPosition = player.FloorCoordinate;
                    WizardsIngame++;
                }
                if (player.IsMe)
                {
            ...
            if (actors.Count() > 0)
            {
                ...
                {
                    float dist = actor.FloorCoordinate.XYDistanceTo(PlayerPosition);
                    if (dist < mindist)
                    {
                        closest = actor;
                        mindist = dist;
